...
class Pdf
{
     ...
     *
     * @param array $mpdfConfig app pdf config
     */
    protected function addCustomFontsConfig($mpdfConfig)
    {
        $config = $this->config;
        if (!isset($config['font_path']) || !isset($config['font_data'])) {
            return $mpdfConfig;
        }
        ...
        $mpdfConfig['fontDir']  = array_merge($fontDirs, [$config['font_path']]);
        $mpdfConfig['fontdata'] = array_merge($fontData, $config['font_data']);
